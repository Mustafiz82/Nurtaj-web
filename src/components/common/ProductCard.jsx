...
const ProductCard = ({ item }) => {
    ...
    return (
        <div className='flex flex-col h-full justify-between'>
            <div>
                <div className='relative'>
                    ...
                    <div className='absolute flex m-3 gap-2 text-white top-0'>
                        {
                            item?.tags?.map((item, idx) => <button key={idx} className={`p-1 font-bold rounded-[3px]  text-[12px] ${item == "Local" ? "bg-[#174A7A]" : "bg-primary "}`}>
                                {item}
                            </button>)
                        }
                    </div>
            ...
            <button className='border w-full mt-2 border-black py-2 px-5 justify-center items-center hover:bg-black duration-300 text-black hover:text-white'>
                Add To Cart
            </button>
        </div>
    );
